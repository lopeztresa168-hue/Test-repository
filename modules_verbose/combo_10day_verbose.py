...
# combo_10day.py (sha256 مرجع: c3aa827684c65f46947453e20eda1b2c1404fe6774d13bd15022836a32d64135) تولید شده است.
...
def process_analysis(trades_json_path, news_dir, interval, target_coin,
    ...
    # [VERBOSE] فهرست کوین‌های OHLC و تعداد روز کاری هر کدام — برای تطبیق مستقیم
    ...
    # [VERBOSE] توزیع نمادها در فایل خام معاملات (قبل از هر فیلتری)
    ...
    # [VERBOSE] شمارش مستقل و فقط-خواندنیِ مراحل فیلتر (کوین → سشن). این پاس
    ...
    # [VERBOSE] تعداد و هش پایدار رویدادهای خبریِ parse‌شده (مستقل از ترتیب بارگذاری)
    ...
    # [VERBOSE] توزیع تعداد معامله در هر دوره (فقط گزارش)
    ...
    if jsonl_out:
        ...
        # [VERBOSE] شمارش مستقل دوره‌هایی که با --min-sample-count حذف می‌شوند
        ...
        for period_key, (status_dict, ret, start_date, end_date) in period_status.items():
            ...
            # [VERBOSE] توزیع رژیم بازار معاملات همین دوره
            ...
            for market_regime, bucket_profits in regime_buckets.items():
                ...
                if _VERBOSE_os.environ.get("VERBOSE_LOG_FULL_RECORDS", "1") != "0":
                    _VERBOSE_log("[VERBOSE][رکورد خروجی] " + json.dumps(records[-1], ensure_ascii=False, default=str))

        # [VERBOSE] خلاصه‌ی تعداد رکورد نهایی به‌ازای رژیم بازار، قبل از نوشتن JSONL
        _VERBOSE_by_regime = {}
        for _vr in records:
...
def main():
    ...
    _VERBOSE_dump_args(args)  # [VERBOSE]
